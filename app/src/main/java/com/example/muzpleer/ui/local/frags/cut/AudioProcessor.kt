...
class AudioProcessor {
    ...
    private var playbackStartedCallback: (() -> Unit)? = null // Новый callback
    ...
    fun extractAmplitudes(audioPath: String, stepMs: Int): List<AmplitudePoint> {
        return try {
            ...
            Log.d(
                TAG,
                "@@AudioProcessor extractAmplitudes Error extracting amplitudes with MediaExtractor, using fallback error = ${e.message}"
            )
            extractAmplitudesFallback(audioPath, stepMs)
        }
    }

    private fun extractAmplitudesWithMediaExtractor(
        audioPath: String,
        stepMs: Int
    ): List<AmplitudePoint> {
        ...
        try {
            ...
            Log.d(
                TAG, "@@AudioProcessor extractAmplitudesWithMediaExtractor " +
                        "Sample rate: $sampleRate, Channels: $channelCount, Duration: $durationSeconds sec"
            )
            // Извлекаем сырые амплитуды
            val rawAmplitudes=
                extractRawAmplitudes(extractor, sampleRate, channelCount, durationSeconds, stepMs)
    ...
    private fun findAudioTrack(extractor: MediaExtractor): Int {
        ...
        for (i in 0 until extractor.trackCount) {
            ...
            if (mimeType?.startsWith("audio/") == true) {
                Log.d(
                    TAG,
                    "@@AudioProcessor findAudioTrack Found audio track at index $i: $mimeType"
                )
                return i
            }
        }
        ...
        Log.e(
            TAG,
            " @@AudioProcessor findAudioTrack No audio tracks found in ${extractor.trackCount} tracks"
        )
        return -1
    }

    //Улучшенный расчет амплитуды с RMS
    private fun calculateAmplitudeFromBuffer(
        buffer: ByteBuffer,
        bytesRead: Int,
        channelCount: Int
    ): Float {
    ...
    fun startPlayback(startTime: Float, endTime: Float) {
            ...
            mediaPlayer = MediaPlayer().apply {
                try {
                    ...
                    setOnPreparedListener {
                        ...
                        // Уведомляем о начале воспроизведения
                        playbackStartedCallback?.invoke()

                        // Устанавливаем автоматическую остановку в endTime
                        if (endTime > startTime) {
                            val durationMs = ((endTime - startTime) * 1000).toLong()
                            Handler(Looper.getMainLooper()).postDelayed({
                                try {
                                    ...
                                    Log.e(TAG, "Error in playback completion", e)
                                }
                            }, durationMs)
                        }
                    }
    ...
    fun setPlaybackStartedCallback(callback: () -> Unit) {
        playbackStartedCallback = callback
    }
...
//        }
    ...
    fun getCurrentPosition(): Float {
        return try {
            if (mediaPlayer != null && isPrepared) {
                mediaPlayer!!.currentPosition.toFloat() / 1000f // возвращаем в секундах
            } else {
                0f
            }
        } catch (e: IllegalStateException) {
