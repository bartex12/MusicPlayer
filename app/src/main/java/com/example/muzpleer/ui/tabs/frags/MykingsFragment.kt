...
class MykingsFragment: BaseFragment()  {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        baseViewModel.getMyTracks().observe(viewLifecycleOwner) { list ->
            val data = list.filter { it.typeFromIfMy == Constants.LITTLE_KING }
            Log.d(TAG, "***MytracksFragment onViewCreated: data = " +
                    " ${data.filter { it.typeFromIfMy ==Constants.LITTLE_KING}.map { it.title }}")
